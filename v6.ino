/*
  ...
  基于网格的流体显示 - 16x16, 32x32, 或 64x64 像素正八边形显示
  物理模拟为圆形，渲染为正八边形
*/
...
void initOctagonParams() {
    // 对于SIM_RESOLUTION x SIM_RESOLUTION的网格
    // 正八边形: 水平/垂直边为8像素长 -> 半长为4像素
    // 这意味着从中心到垂直/水平边中点的距离是4像素
    
    // 如果SIM_RESOLUTION=16，中心在7.5（0-15的中间）
    // 但最好用整数计算
    ...
    // 水平边半长 = 4像素 (总长度8像素)
    // 但这是以像素为单位的，我们需要归一化到0-1范围用于模拟映射
    
    // 实际上我们直接在像素坐标中判断
    // 对于16x16: 中心在7.5, 水平边从x=3.5到x=11.5 (长度8)
    // 垂直边同理
    // 对角边: 从(3.5, 7.5±4)到(7.5±4, 3.5) 这样的关系
    
    // 简化: 使用距离中心的L1范数（曼哈顿距离）的变形
    // 正八边形可以定义为: max(|x-cx|, |y-cy|) + (sqrt(2)-1)*min(|x-cx|, |y-cy|) <= R
    ...
    // 计算八边形"半径"（中心到边的距离）
    // 对于边长为s的正八边形，外接圆半径 R = s/2 * sqrt(4+2*sqrt(2))/2
    // 但这里s=8(水平边)，我们需要的是内切相关参数
    
    // 简单方法: 使用截角正方形
    // 从正方形切掉四个角，切掉的部分形成等腰直角三角形，斜边=5像素 -> 直角边=5/sqrt(2)≈3.5像素
    
    // 直接设置:
    // 中心 (center, center)
    // 水平边范围: [center-4, center+4] 在 y = center ± (4+3.5) 的位置? 不对
    
    // 重新定义:
    // 对于16x16网格，正八边形顶点坐标（近似）:
    // (4, 0), (12, 0), (16, 4), (16, 12), (12, 16), (4, 16), (0, 12), (0, 4)
    // 但这是16x16的，边长: 顶边从x=4到x=12，长度8 ✓
    // 斜边: 从(12,0)到(16,4)，长度sqrt(16+16)=sqrt(32)≈5.66，接近5但不精确
    
    // 如果要精确斜边为5，需要调整:
    // 斜边5像素意味着 dx=dy=5/sqrt(2)≈3.54，不是整数
    
    // 改用整数步长: 斜边占3x3的对角线，长度sqrt(18)≈4.24，也不对
    
    // 实际OLED像素是方的，我们直接用L1范数定义八边形边界
    // 正八边形条件: |x-cx| + |y-cy| <= R 且 max(|x-cx|, |y-cy|) <= R2
    
    // 简化为: 在网格坐标(i,j)中，0 <= i,j < SIM_RESOLUTION
...
bool isInsideOctagon(int i, int j) {
    ...
    // 正八边形: 截角正方形
    // 定义: dx <= halfSize 且 dy <= halfSize 且 dx + dy <= halfSize + (halfSize - cornerCut)
    // 即: 在正方形内，但角被切掉
