...
void drawFluid() {
    ...
    // ========== 关键：流体像素左下偏移量 ==========
    // X_SHIFT：左移为负数（-1=左移1物理像素，-2=左移2）
    // Y_SHIFT：下移为正数（1=下移1物理像素，2=下移2）
    // 缺2个像素就把两个值都改成2，亲测1基本解决问题
    const int X_SHIFT = -3;
    const int Y_SHIFT = 4;
    ...
    for (int i = 0; i < SIM_RESOLUTION; i++) {
        for (int j = 0; j < SIM_RESOLUTION; j++) {
            ...
            int simI = (int)((float)i / SIM_RESOLUTION * (fNumX - 2));
            int simJ = (int)((float)j / SIM_RESOLUTION * (fNumY - 2));
