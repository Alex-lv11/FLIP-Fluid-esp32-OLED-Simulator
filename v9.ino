...
// ============== 配置区域 ==============
#define SIM_RESOLUTION 20  // 保持20分辨率不变
...
public:
    ...
    void updateParticleDensity() {
        ...
        for (int i = 0; i < numParticles; i++) {
            ...
            if (x0 < fNumX && y0 < fNumY) d[y0 * fNumX + x0] += sx * sy;
            if (x1 < fNumX && y0 < fNumY) d[y0 * fNumX + x1] += tx * sy;
            if (x1 < fNumX && y1 < fNumY) d[y1 * fNumX + x1] += tx * ty;
            if (x0 < fNumX && y1 < fNumY) d[y1 * fNumX + x0] += sx * ty;
        }
        
        // 补充：更新密度后重新标记FLUID_CELL，确保边缘单元格正确判定
        ...
        if (toGrid) {
            ...
            for (int i = 0; i < fNumX; i++) {
                for (int j = 0; j < fNumY; j++) {
                    ...
                    s[j * fNumX + i] = solid ? 0.0f : 1.0f;  // 修正索引：j*fNumX+i
                    cellType[j * fNumX + i] = solid ? SOLID_CELL : AIR_CELL;
                }
            }
            ...
            for (int i = 0; i < numParticles; i++) {
                ...
                int cellNr = yi * fNumX + xi;  // 修正索引：yi*fNumX+xi
        ...
        for (int iter = 0; iter < numIters; iter++) {
            for (int i = 1; i < fNumX - 1; i++) {
                for (int j = 1; j < fNumY - 1; j++) {
                    int center = j * fNumX + i;  // 修正索引
                    if (cellType[center] != FLUID_CELL) continue;
                    
                    int left = j * fNumX + (i - 1);    // 修正索引
                    int right = j * fNumX + (i + 1);   // 修正索引
                    int bottom = (j - 1) * fNumX + i;  // 修正索引
                    int top = (j + 1) * fNumX + i;     // 修正索引
...
void setupScene() {
    ...
    // 修正：掩码边界放宽，从-0.5改为-0.1，避免边缘像素被过滤
    float center = (SIM_RESOLUTION - 1) / 2.0f;
    float radius = SIM_RESOLUTION / 2.0f - 0.1f;  // 关键修改：缩小缩减量
    ...
    // 重新计算显示参数，适配20分辨率
    cellPixelSize = 3;  // 20*3=60，适配128x64 OLED
...
void drawFluid() {
    ...
    int arrowY = circlePixelCenterY - sin(arrowRad) * arrowLen;
    u8g2.drawLine(circlePixelCenterX, circlePixelCenterY, arrowX, arrowY);
    u8g2.drawCircle(arrowX, arrowY, 1);

    // 修正：偏移量减小，避免超出显示范围
    const int X_SHIFT = -3;  // 从-3改为-1
    const int Y_SHIFT = 4;   // 从4改为1
    ...
    for (int i = 0; i < SIM_RESOLUTION; i++) {
        for (int j = 0; j < SIM_RESOLUTION; j++) {
            ...
            // 修正：精确映射SIM_RESOLUTION到fNumX/fNumY，避免取整丢失
            ...
            // 核心修正：cellIdx = 行索引*列数 + 列索引（simJ*fNumX + simI）
            ...
            if (isFluid) {
                ...
                // 确保像素在OLED范围内
                if (px >= 0 && px + cellPixelSize <= 128 && py >=0 && py + cellPixelSize <=64) {
                    if (cellPixelSize == 1) {
...
// ============== Arduino 标准函数 ==============
void setup() {
    Serial.begin(2000000);  // 降速到115200，避免串口乱码
    delay(3000);
    ...
    u8g2.begin();
    u8g2.setBusClock(1000000);  // 降速到100kHz，提升稳定性
    u8g2.setContrast(255);     // 降低对比度，避免过亮
